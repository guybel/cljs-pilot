...
(defonce state
  (atom {:heading-error-int      0
         ...
         :servo-last-time          0.0
         :servo-min-pulse-ms       800   ; duree mini d'une poussee, pour laisser le temps au verin de vaincre la stiction et d'atteindre sa vitesse nominale
         :servo-max-speed          1.0}))
      ...
      (let [pilot-name (v/get-value "ap.pilot")
        ...
        (if pilot
          (let [cmd (basic/process! pilot ap-state)
            ...
            (js/console.debug "[autopilot] err=" (:heading-error ap-state)
                               "cmd=" cmd "out=" out)
            (servo/send-command! out))
          (js/console.warn "[autopilot] Pilote introuvable pour ap.pilot="
                            pilot-name "(cles disponibles:" (keys pilots) ")"))))))
